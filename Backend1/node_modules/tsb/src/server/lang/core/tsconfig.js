...
function getProjectSync(pathOrSrcFile) {
    ...
    try {
        ...
        if (err.message == "not found") {
            var bland = utils_1.makeBlandError(fsu.consistentPath(pathOrSrcFile), err.message);
            throw errorWithDetails(new Error(exports.errors.GET_PROJECT_NO_PROJECT_FOUND), { projectFilePath: fsu.consistentPath(pathOrSrcFile), error: bland });
        }
    }
    ...
    if (res.data) {
        ...
        throw errorWithDetails(new Error(exports.errors.GET_PROJECT_JSON_PARSE_FAILED), { projectFilePath: projectFilePath, error: json.parseErrorToCodeError(projectFilePath, res.error) });
    }
    ...
    if (toExpand) {
        try {
            ...
            throw errorWithDetails(new Error(exports.errors.GET_PROJECT_GLOB_EXPAND_FAILED), { projectFilePath: projectFilePath, error: utils_1.makeBlandError(projectFilePath, ex.message) });
        }
    }
    if (projectSpec.filesGlob) {
    ...
    if (validationResult.errorMessage) {
        throw errorWithDetails(new Error(exports.errors.GET_PROJECT_PROJECT_FILE_INVALID_OPTIONS), { projectFilePath: projectFilePath, error: utils_1.makeBlandError(projectFilePath, validationResult.errorMessage) });
    }
