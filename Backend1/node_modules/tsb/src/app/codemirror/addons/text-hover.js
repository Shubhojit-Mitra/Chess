...
(function () {
    ...
    function showTooltip(e, content) {
        ...
        function position(e) {
            if (!tt.parentNode)
                return exports.CodeMirror.off(document, "mousemove", position);
            tt.style.top = Math.max(0, e.clientY - tt.offsetHeight - 5) + "px";
            tt.style.left = (e.clientX + 5) + "px";
        }
        exports.CodeMirror.on(document, "mousemove", position);
        position(e);
    ...
    function showTooltipFor(e, content, node, state, cm) {
        ...
        function hide() {
            exports.CodeMirror.off(node, "mouseout", hide);
            exports.CodeMirror.off(node, "click", hide);
        ...
        exports.CodeMirror.on(node, "mouseout", hide);
        exports.CodeMirror.on(node, "click", hide);
    ...
    function parseOptions(cm, options) {
        ...
        if (!options.getTextHover)
            options.getTextHover = cm.getHelper(exports.CodeMirror.Pos(0, 0), "textHover");
        if (!options.getTextHover)
        ...
        if (node) {
            ...
            result.then(function (content) {
                if (PromiseQueue[PromiseQueue.length - 1] !== result) {
                    PromiseQueue = PromiseQueue.filter(function (p) { return p != result; });
                    return;
                }
                PromiseQueue = PromiseQueue.filter(function (p) { return p != result; });
                if (content) {
    ...
    function optionHandler(cm, val, old) {
        if (old && old != exports.CodeMirror.Init) {
            exports.CodeMirror.off(cm.getWrapperElement(), "mouseover", cm.state.textHover.onMouseOver);
            delete cm.state.textHover;
        }
        if (val) {
            var state = cm.state.textHover = new TextHoverState(cm, parseOptions(cm, val));
            exports.CodeMirror.on(cm.getWrapperElement(), "mouseover", state.onMouseOver);
        }
    }
    ...
    exports.CodeMirror.defineOption("textHover", false, optionHandler);
})();
